...
- **Permission Alignment**: Evaluates specific action permission (`hasRoleActionPermission`) OR master update rights (`hasRolePermission(..., 'canUpdate')`). Never fails an action solely for lacking generic record update permissions.

---

## Pattern: Dynamic Session Cryptography & Zero-Lookup Auth Gate

Use for authenticating all protected requests (`doPost`) without redundant sheet or database table lookups.

- **Dynamic Rolling Proof**: Every protected request sends `{ token, sessionKey }`. The `sessionKey` is generated by the client from parameters derived directly from the token UUID segments (`genPrime`, `letterShift`, `digitShift`, `genOffset`) and its local rolling generation counter.
- **Single-Gate Verification**: `verifySessionProof(token, sessionKey)` runs at the dispatcher entrance in `doPost`. It validates that the generation is within a bounded window (`WINDOW = 2`) to support simultaneous/in-flight requests.
- **Unified Session Cache**: Successful login seeds `AQL_SESSION_<SpreadsheetId>_<token>` in `CacheService` with `{ uuid_code, generation, user, roleIds, accessRegionScope, rowNumber, v }`.
- **Zero-Lookup Execution**: Once verified, the dispatcher uses `proof.auth` directly. Handlers bypass redundant row-by-row role permission scans during write/action execution while preserving record-level policies (Owner, AccessRegion) and schema validations.
- **Instant Mutation & Version Busting**: Profile mutations (`handleUpdateAvatar`, `handleUpdateName`, `handleUpdateEmail`) update the session cache object immediately. Sheet-level admin edits bump the global `AQL_SESSION_AUTHVER_<SpreadsheetId>` to bust cached sessions.
